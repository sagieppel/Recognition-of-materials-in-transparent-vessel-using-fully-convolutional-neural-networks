...
########################################Build Net#####################################################################################################################
    def build(self, rgb,NUM_CLASSES,keep_prob):  #Build the fully convolutional neural network and load weight for decoder based on trained VGG16 network
        """
...
#-----------------------------Build network encoder based on VGG16 network and load the trained VGG16 weights-----------------------------------------
       #Layer 1
        self.conv1_1 = self.conv_layer(bgr, "conv1_1") #Build Convolution layer +Relu and load weights
...
#-------------------------------------Build Decoder --------------------------------------------------------------------------------------------------
       ...
       #Split final probability map to set of categories in given granularity
        self.VesselProb, self.PhaseProb, self.LiquidSolidProb, self.AllPhasesProb = tf.split(self.conv_t3, [2, 3, 4, 15], 3)
#--------------------Transform  probability vectors to label maps-----------------------------------------------------------------
        self.AllPhasesPred = tf.argmax(self.AllPhasesProb, dimension=3, name="AllPhasesPred")
